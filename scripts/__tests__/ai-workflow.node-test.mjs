...
function fixture(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "bitsocial-react-hooks-workflow-"));
...
test("shared sources generate valid, deterministic harness outputs", (t) => {
  ...
  for (const harness of ["claude", "cursor"]) {
    assert.equal(
      readFrontmatter(before.get(`.${harness}/agents/reviewer.md`)).metadata.model,
      undefined,
    );
  }
});
...
test("unknown role metadata fails instead of silently changing model inheritance", (t) => {
  ...
  const content = fs
    ...
    .replace("name: reviewer", "name: reviewer\nmodel: undocumented-model");
